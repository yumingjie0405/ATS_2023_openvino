...
bool Autoaim::updateSpinScore() {
    for (auto score = spin_score_map.begin(); score != spin_score_map.end();) {
        ...
        LOG(INFO) << "[SpinDetection] Current Spin score :" << (*score).first << " : " << (*score).second << " "
                  << spin_status;
        // 若分数过低移除此元素
...
//    Eigen::Matrix3d rmat_imu = src.quat.toRotationMatrix();
    ...
    ///------------------------将对象排序，保留面积较大的对象---------------------------------
    sort(objects.begin(), objects.end(), [](ArmorObject &prev, ArmorObject &next) { return prev.area > next.area; });
            ...
            if (same_armors_cnt == 2) {
                ...
                if (new_tracker != nullptr && last_tracker != nullptr) {
                    ...
                    LOG(INFO) << "[SpinDetection] Candidate Spin Movement Detected : " << cnt.first << " : "
                              << spin_movement;
                    if (abs(spin_movement) > 10 && new_armor_timestamp == src.timestamp &&
                        last_armor_timestamp == src.timestamp) {
...
//        computeAspectRatio(armor);
        ...
        if (armor.color == 2)
            putText(src.img, fmt::format("N{}", armor.id), armor.apex2d[0], FONT_HERSHEY_SIMPLEX, 1, {255, 255, 255},
                    2);
        if (armor.color == 3)
...
#ifdef SHOW_FPS
    putText(src.img, fmt::format("FPS: {}", int(1000 / dr_full_ms)), {10, 25}, FONT_HERSHEY_SIMPLEX, 1, {199, 255, 42});
    putText(src.img, fmt::format("YAW: {:.4f}", angle[0]), {10, 50}, FONT_HERSHEY_SIMPLEX, 1, {255, 255, 0});
